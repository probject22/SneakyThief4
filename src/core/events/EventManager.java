...
/**
 *
 * The EventManager at every action that the SpriteManager makes, whether a new
 * event should be generated, and if so, it passes this on to the appropriate
 * agent.
 *
...
public class EventManager {
	...
	/**
	 * checks if the actionElement performed by the given agent triggers any
	 * events. If so the events are routed to the appropriate agents. Always
	 * trigger events AFTER EXECUTION OF THE ACTIONELEMENT!
	 * 
	 * @param actionElement
	 * @param agent
	 */
	public void triggerEvent(ActionElement actionElement, Agent agent) {
		...
		/* this handles the move actionElement (THIS NEEDS TO BE IMPLEMENTED) */
			...
			// TODO dont trigger this event when a guard is entering a tower
			// leaving a tower or when the agent turns for more then 45 degrees
			// (page 4)
			if (map.getCopyOfMap()[agent.getCoordinates().x][agent
					.getCoordinates().y] == GridState.Door)
				generateDoorSoundEvent(agent, timeStamp);
			if (map.getCopyOfMap()[agent.getCoordinates().x][agent
					.getCoordinates().y] == GridState.Window)
				generateWindowSoundEvent(agent, timeStamp);

			generateVisionEvent(agent, timeStamp);
			generateMoveSoundEvent(agent, timeStamp,
					((Move) actionElement).speed);
		}
	...
	private Boolean isInRadius(Coordinate target, Coordinate baseCoord, double r) {
		double distence = Coordinate.distanceBetweenCoordinates(target,
				baseCoord);
		return distence <= r;
	}

	private Boolean isInView(Coordinate target, double angle, double r,
	...
	/**
	 * Looks at four different conditions to find if the corners that have the
	 * largest angles from the agent(base).
	 * 
	 * @param base
	 *            : agents position
	 * @param block
	...
	private double getBlockingAngle(Coordinate base, Coordinate block) {
		...
		if ((base.x > block.x && base.y > block.y)
				|| (base.x < block.x && base.y < block.y))
			return Math.abs(base.getAngleFromAgent(new Coordinate(block.x,
					block.y + 1, 0))
					- base.getAngleFromAgent(new Coordinate(block.x + 1,
							block.y, 0)));
		if ((base.x < block.x && base.y > block.y)
				|| (base.x > block.x && base.y < block.y))
			return Math.abs(base.getAngleFromAgent(new Coordinate(block.x,
		...
		if (base.x == block.x && base.y < block.y)
			return Math.abs(base.getAngleFromAgent(new Coordinate(block.x,
					...
					- base.getAngleFromAgent(new Coordinate(block.x + 1,
							block.y, 0)));
		if (base.x == block.x && base.y > block.y)
			return Math.abs(base.getAngleFromAgent(new Coordinate(block.x + 1,
					block.y + 1, 0))
					- base.getAngleFromAgent(new Coordinate(block.x,
							block.y + 1, 0)));
	...
	private void generateVisionEvent(Agent agent, double timeStamp) {
		...
		// Adding the states (towers,walls,trees) in the vision to the Grid hash
		// map
		GridState[][] state = map.getCopyOfMap();
		vision.addGrid(new Coordinate(baseCoords.x, baseCoords.y, 0),
				GridState.path);
		for (int i = 0; i < map.getMapWidth(); i++) {
			for (int j = 0; j < map.getMapHeight(); j++) {

				if (!isInView(new Coordinate(i, j, 0), visionAngle,
						minVisionRange, baseCoords)
						&& isInView(new Coordinate(i, j, 0), visionAngle,
								maxVisionRange, baseCoords))
					...
					if (state[i][j] == GridState.Sentry)
						if (!isInView(new Coordinate(i, j, 0), visionAngle,
								maxVisionRange, baseCoords)
								&& isInView(new Coordinate(i, j, 0),
										visionAngle, towerVisionRange,
										baseCoords))
							vision.addGrid(new Coordinate(i, j, 0), state[i][j]);
					...
					if (state[i][j] == GridState.Door
						...
						if (!isInView(new Coordinate(i, j, 0), visionAngle,
								maxVisionRange, baseCoords)
								&& isInView(new Coordinate(i, j, 0),
										visionAngle, structureVisionRange,
										baseCoords))
							vision.addGrid(new Coordinate(i, j, 0), state[i][j]);
		...
		// Goes through all the Grids in the view Cone which does not care about
		// walls
		for (Coordinate barrier : vision.getStateInVisionMap().keySet()) {
			if (vision.getStateInVisionMap().get(barrier) == GridState.Wall) {
				// Gets the angle of the left side of the barrier - right side
				// of the barrier
				...
				for (Coordinate gridCheck : vision.getStateInVisionMap()
						.keySet()) {
					if (!isInView(gridCheck, blockAngle,
							baseCoords.distance(barrier), newBase)
							&& isInView(gridCheck, blockAngle, maxVisionRange,
									newBase)) {
					...
					for (Coordinate neighbor : barrier.neighbourCoordinates()) {
						if (neighbor.x >= 0 && neighbor.y >= 0
								&& neighbor.x < state.length
								&& neighbor.y < state[0].length)
							if (state[neighbor.x][neighbor.y] == GridState.Wall) {
								double nblockAngle = getBlockingAngle(
										baseCoords, neighbor);
								Coordinate nNewBase = baseCoords.clone();
								nNewBase.angle = baseCoords.getAngle(neighbor);
								if (!isInView(gridCheck, nblockAngle,
										baseCoords.distance(neighbor), nNewBase)
										&& isInView(gridCheck, nblockAngle,
												maxVisionRange, nNewBase)) {
									remove.add(gridCheck);
								}
							}
					}
		...
		vision.addGrid(new Coordinate(coordinate.x, coordinate.y, 0),state[coordinate.x][coordinate.y]);

		// Adding the sprite in the vision to the Sprite hash map
		for (Sprite sprite : spriteManager.getAgentList()) {
			for (Coordinate tempCoord : vision.getStateInVisionMap().keySet()) {
				Coordinate spriteCoord = sprite.getCoordinates();
				if (tempCoord.x == spriteCoord.x
						&& tempCoord.y == spriteCoord.y)
					vision.addSprite(sprite.getCoordinates().clone(), sprite);
			}
		}
	...
	public void initializeSoundOnMap() {
		...
		for (int i = 0; i < nWidth; i++) {
			for (int j = 0; j < nHeight; j++) {
				...
				soundCoordinates[k][1] = new Coordinate((i + 1) * 25,
						(j + 1) * 25, 0);
				k++;
			}
		}
		...
		if (extraHeight != 0) {
			for (int i = 0; i < nWidth; i++) {
				tick[k][0] = 0;
				tick[k][1] = generateNextSound();
				soundCoordinates[k][0] = new Coordinate(i * 25,
						(nHeight - 1) * 25, 0);
				soundCoordinates[k][1] = new Coordinate((i + 1) * 25, nHeight
						* extraHeight, 0);
				k++;
			}
		}
		...
		if (extraWidth != 0) {
			for (int j = 0; j < nHeight; j++) {
				tick[k][0] = 0;
				tick[k][1] = generateNextSound();
				soundCoordinates[k][0] = new Coordinate((nWidth - 1) * 25,
						j * 25, 0);
				soundCoordinates[k][1] = new Coordinate(nWidth * extraWidth,
						(j + 1) * 25, 0);
				k++;
			}
		}
	...
	public void randomSoundOnMap(double plusTick) {
		...
		for (int i = 0; i < nSections; i++) {
			generateRandomSound(i, plusTick, soundCoordinates[i][0],
					soundCoordinates[i][1]);
		}
	}

	// for one 25m2 area
	private void generateRandomSound(int nrOfArea, double plusTick,
		...
		if (tick[nrOfArea][0] >= tick[nrOfArea][1]) { // Coordinate.distanceBetweenCoordinates()
			for (Agent tempAgent : spriteManager.getAgentList()) {
				Coordinate soundCoo = new Coordinate((int) Math.random()
						* (bottomLeft.x - topRight.x), (int) Math.random()
						* (bottomLeft.y - topRight.y), 0);
				if (Coordinate.distanceBetweenCoordinates(soundCoo,
						tempAgent.getCoordinates()) <= distanceSound) {
					// calculate the angle under which the agent hears the sound
					// and add the uncertainty to it
					...
					double direction = (Math.atan2(dy, dx) + (Math.random()
							* (varianceRad * 2) - varianceRad))
							% 2 * Math.PI;
					if (direction < 0)
						direction += 2 * Math.PI;

					Sound sound = new Sound(tempAgent.getTimeKey(), direction,
							tempAgent.getCoordinates().clone());
	...
	private void generateMoveSoundEvent(Agent agent, double timeStamp,
			double speed) {
	...
	private void generateSoundEvent(Agent agent, double timeStamp,
			double distance) {
		/* in degree */
		...
		for (Agent tempAgent : spriteManager.getAgentList())
			if (distenceBetweenAgents(agent, tempAgent) <= distance) {
				/*
				 * calculate the angle under witch the agent hears the sound and
				 * add the variance to it
				 */
				double dx = agent.getCoordinates().x
						- tempAgent.getCoordinates().x;
				double dy = agent.getCoordinates().y
						- tempAgent.getCoordinates().y;
				double direction = (Math.atan2(dy, dx) + (Math.random()
						* (varianceRad * 2) - varianceRad))
						% 2 * Math.PI;
				if (direction < 0)
				...
				Sound sound = new Sound(timeStamp, direction, tempAgent
	...
	private double distenceBetweenAgents(Agent agent1, Agent agent2) {
		return Coordinate.distanceBetweenCoordinates(agent1.getCoordinates(),
				agent2.getCoordinates());
	}
}

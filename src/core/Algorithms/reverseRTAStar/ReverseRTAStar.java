...
/**
 *
 * Class that generically performs the Real time AStar algorithm. It takes a
 * neighbourGenerator that generates the neighbours of a node, a CostFunction
 * that should evaluates the actual cost between two nodes, and a
 * HeuristicFunction that evaluates the heuristic cost between two nodes. This
 * way we can change the heuristic without touching this class.
 *
 * To change the heuristic, look at the MapHeuristicFunction class. To change
 * the cost function, look at the MapCostFunction class.
 *
 * To apply on something else than the map, make new classes that implement
 * HeuristicFunction, CostFunction, and NeighbourGenerator.
 *
